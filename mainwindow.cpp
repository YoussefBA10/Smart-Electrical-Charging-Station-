...
void MainWindow::on_pushButton_historique_clicked()
{
    ...
    QSqlQuery query;
    query.prepare("SELECT ID_VOITURE, ID_CLIENT, MATRICULE, MODEL, ID_SESSION  FROM VOITURE");
  ...
  qDebug() << "TEST PRIX!" <<PRICE ;
   ...
   painter.drawText(tableX + cellWidth * 47, tableY, "TIME");
    ...
    while (query.next() && query2.next() && query1.next()) {
       ...
       painter.drawText(tableX + cellWidth * 46, tableY + row * cellHeight, QDateTime::currentDateTime().toString());
