...
QString nom=emp.get_nom();
    Smtp* smtp = new Smtp("mohamedjasser.yahmadi@esprit.tn", "211JMT9785", "smtp.gmail.com");
    connect(smtp, SIGNAL(status(QString)), this, SLOT(mailSentmdp(QString)));
    smtp->sendMail("mohamedjasser.yahmadi@esprit.tn", emailtr, " Mot de passe", "Notre société TCHARGI vous souhaite la bienvenue Monsieur/Madame :   " + QString(nom.at(0)).toUpper() + nom.mid(1) + " " + QString(prenom.at(0)).toUpper() + prenom.mid(1) + "\n\nVotre mot de passe est : " + mdp + "\n\nNous vous conseillons de le changer immédiatement après votre première connexion.");
